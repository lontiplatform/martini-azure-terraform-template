...
resource "azurerm_monitor_metric_alert" "app_gw_unhealthy_hosts" {
  name                = "${local.name_prefix}-appgw-unhealthy-hosts"
  resource_group_name = azurerm_resource_group.rg.name
  scopes              = [module.app_gw.application_gateway_id]
  description         = "Application Gateway reports one or more unhealthy backend hosts."
  severity            = 1
  frequency           = "PT1M"
  window_size         = "PT5M"
...
resource "azurerm_monitor_metric_alert" "app_gw_5xx_burst" {
  name                = "${local.name_prefix}-appgw-5xx-burst"
  resource_group_name = azurerm_resource_group.rg.name
  scopes              = [module.app_gw.application_gateway_id]
  description         = "Application Gateway returned more than 5 HTTP 5xx responses in 5 minutes."
  severity            = 2
  frequency           = "PT1M"
  window_size         = "PT5M"
...
resource "azurerm_monitor_metric_alert" "app_gw_failed_requests" {
  name                = "${local.name_prefix}-appgw-failed-requests"
  resource_group_name = azurerm_resource_group.rg.name
  scopes              = [module.app_gw.application_gateway_id]
  description         = "Application Gateway failed-request count exceeded 10 in 5 minutes."
  severity            = 2
  frequency           = "PT1M"
  window_size         = "PT5M"
